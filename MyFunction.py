...
class Perceptron():
    ...
    def CalculateOutput(self,iPattern):
        ...
        self.var_energy_eLTP += self.energy_scale_maintenance * np.sum( np.fabs(self.arr_deltaW[np.arange(0,self.nDim)]) )
    ...
    def SaveChange(self):
        ...
        self.var_energy += self.energy_scale_lLTP * np.sum( np.fabs(self.arr_deltaW[np.arange(0,self.nDim)]) ) # assume changing bias costs no energy
        self.var_energy_lLTP += self.energy_scale_lLTP * np.sum( np.fabs(self.arr_deltaW[np.arange(0,self.nDim)]) ) # assume changing bias costs no energy
        self.arr_deltaW = np.zeros(self.nDim+1)
            ...
            for iPattern in range(0,self.nPattern):
                ...
                for iArrSyn in range(0,len(arr_syn_update)):
                    ...
                    if(nSynapse!=self.nDim):
                        self.var_energy += self.energy_scale_lLTP * np.fabs(self.arr_deltaW[nSynapse]) # assume changing bias costs no energy
                        self.var_energy_lLTP += self.energy_scale_lLTP * np.fabs(self.arr_deltaW[nSynapse]) # assume changing bias costs no energy
